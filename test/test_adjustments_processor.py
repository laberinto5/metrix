"""
Unit tests for adjustments_processor module.
"""
...
class TestLoadAdjustments:
    """Tests for load_adjustments."""
    
    def test_load_adjustments_valid(self):
        """Test loading valid adjustments."""
        adjustments_data = {
    ...
    def test_load_adjustments_invalid_json(self):
        """Test that raises error with invalid JSON."""
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8') as f:
    ...
    def test_load_adjustments_invalid_key(self):
        """Test that raises error with invalid key."""
    ...
    def test_load_adjustments_invalid_type(self):
        """Test that raises error with invalid type."""
...
class TestApplyReferenceReplacements:
    ...
    def test_apply_reference_replacements_word_boundaries(self):
        """Test that respects word boundaries."""
        text = "tehater"
        replacements = {"teh": "the"}
        result = apply_reference_replacements(text, replacements)
        assert result == "tehater"  # Should not replace because there's no word boundary
    ...
    def test_apply_reference_replacements_case_sensitive(self):
        ...
        assert result == "Teh cat"  # Does not replace because it's case-sensitive
...
class TestApplyEquivalences:
    ...
    def test_apply_equivalences_case_sensitive(self):
        """Test case-sensitive equivalences."""
        text = "Wanna go"
        equivalences = {"want_to": ["want to", "wanna"]}
        result = apply_equivalences(text, equivalences, case_sensitive=True)
        assert result == "Wanna go"  # Does not replace because it's case-sensitive


class TestApplyCleanUp:
    """Tests for apply_clean_up."""
    ...
    def test_apply_clean_up_normalizes_spaces(self):
        """Test that normalizes spaces after cleaning."""
        text = "uh  hello  ah"
        clean_up_list = ["uh", "ah"]
        result = apply_clean_up(text, clean_up_list)
        assert result == "hello"
    ...
    def test_apply_clean_up_case_sensitive(self):
        """Test case-sensitive cleanup."""
        text = "Uh hello"
        clean_up_list = ["uh"]
        result = apply_clean_up(text, clean_up_list, case_sensitive=True)
        assert result == "Uh hello"  # Does not remove because it's case-sensitive
...
class TestApplyAdjustments:
    ...
    def test_apply_adjustments_full_workflow(self):
        """Test complete adjustments application."""
        ...
        assert "the" in ref_result  # Replacement applied
        assert "want to" in ref_result  # Equivalence applied
        assert "want to" in hyp_result  # Equivalence applied
    
    def test_apply_adjustments_only_reference_replacements(self):
        """Test that reference_replacements only applies to reference."""
        reference = "teh cat"
        hypothesis = "teh cat"
        adjustments = {
            "reference_replacements": {"teh": "the"},
        ...
        assert hyp_result == "teh cat"  # Does not apply to hypothesis
